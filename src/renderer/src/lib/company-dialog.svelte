  ...
  >
    <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        ...
        <button class="text-gray-400 hover:text-gray-500 focus:outline-hidden" onclick={closeModal}>
          <Cross />
        </button>
      </div>
...
{#if showCaptchaModal}
  <div
    class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-xs flex items-center justify-center z-60"
  >
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
      ...
      <div class="space-y-4">
        <div class="flex justify-center">
          <img src={`data:image/png;base64,${captchaImage}`} alt="Captcha" class="border rounded-sm" />
        </div>
        <input
          type="text"
